...
- Album views
- Backend
	- Assign each requested song to a youtube video (optimized to not return music videos that have added distracting parts)
	- Spotify api token distribution
- Full screen view
- Caches recently requests tracks using localstorage
- Settings 
- Streaming audio, programmatically controlls the playback from any part of the application
	- *Note: Octave does not actually play music, only royalty-free audio to demonstrate the playback capabilities*
